...
**Flomo 定位**：不是主业务系统，而是轻量灵感入口。用于快速记录精彩洞察，重要内容后续整理到主系统。
...
- **配 Flomo**：如果你需要随时捕捉灵感和临时想法（可与任意主平台搭配）
...
```bash
# 配置 Flomo MCP
# 1. 访问 https://flomoapp.com/mine?source=incoming_webhook 获取 API URL
# 2. 配置 MCP 客户端:
{
  "mcpServers": {
    "flomo": {
      ...
      "env": {
        "FLOMO_API_URL": "https://flomoapp.com/iwh/xxxxxxxx"
      }
    }
  }
}
...
├── 发现精彩洞察 → coo-knowledge-ingestion（建议做成选题，同时写入 Flomo）
...
| `lark-read-doc.sh` | 读取飞书文档 |
| `lark-create-task.sh` | 创建飞书任务 |
| `lark-query-tasks.sh` | 查询飞书任务 |
...
| `obsidian-read-note.sh` | 读取笔记 |
| `obsidian-update-note.sh` | 更新笔记 |
| `obsidian-query-tasks.sh` | 查询任务 |

### Flomo

| 脚本 | 用途 |
|---|---|
| `flomo-onboarding.sh` | Flomo MCP 配置向导 |
| `flomo-write.sh` | 写入 Flomo 笔记 |
